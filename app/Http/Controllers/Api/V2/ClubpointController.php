...
class ClubpointController extends Controller
{
    ...
    public function convert_into_wallet(Request $request)
    {
        ...
        $club_point = ClubPoint::where('user_id', auth()->user()->id)->first();
        if( $club_point)
        {
            if($club_point?->convert_status == 0) {
                ...
                $wallet->type = 'in'; // Money flowing into the wallet from club points
                $wallet->save();
        ...
        else 
        {
             return response()->json([
                    'success' => false,
                    'message' => translate('user doesnt have points')
                ],400);
        }
