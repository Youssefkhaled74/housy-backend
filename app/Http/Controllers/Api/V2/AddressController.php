...
class AddressController extends Controller
{
    ...
    {
        return new CitiesCollection(City::where('status', 1)->paginate($request->per_page??10));
    }

    public function getStates(Request $request)
    {
        return new StatesCollection(State::where('status', 1)->paginate($request->per_page??10));
    }
    ...
    public function getCountries(Request $request)
    {
        ...
        return new CountriesCollection($countries->paginate($request->per_page??10));
    }
    ...
    public function getCitiesByState($state_id,Request $request)
    {
        ...
        return new CitiesCollection($city_query->paginate($request->per_page??10));
    }
    public function getCitiesBycountrie($id,Request $request)
    {
        $city_query = City::where('status', 1)->where('country_id',$id);
        if ($request->name != "" || $request->name != null) {
            $city_query->where('name', 'like', '%' . $request->name . '%');
        }
        $cities = $city_query->get();
