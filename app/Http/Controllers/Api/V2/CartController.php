   ...
   public function get_cart_products(Request $request)
    ...
    public function getList(Request $request)
    {
            ...
            foreach ($owner_ids as $owner_id) {
                ...
                if (!empty($shop_items_raw_data)) {
                    foreach ($shop_items_raw_data as $shop_items_raw_data_item) {
                        ...
                        $shop_items_data_item["product_name"] = translate($product->name);
        ...
        $paginatedShops = new LengthAwarePaginator(
            $shopsCollection->forPage($page, $perPage)->values(),
        ...
        $products = $productsInCart->getCollection()->pluck('product');

        $productsInCart->setCollection($products);

            return response()->json([
                "grand_total" => single_price($grand_total),
                //"prducts"=>  new ProductDetailCollection($productsInCart),
                 "data" => $paginatedShops
            ]);
    ...
    public function remove(Request $request)
    {
        ...
        if (!$product_id) {
            return response()->json([
                'result' => false,
                'message' => translate('Product ID is required.')
            ], 400);
        }
        ...
        if ($user_id !== null) {
            ...
            if ($temp_user_id === null) {
                return response()->json([
                    'result' => false,
                    'message' => translate('Temporary user ID is required.')
                ], 400);
            }

            $cartItem = Cart::where('temp_user_id', $temp_user_id)
                            ->where('product_id', $product_id)
                            ->where('variation', $variant)
                            ->first();
        }
        ...
        if ($cartItem) {
            ...
            return response()->json([
                'result' => true,
                'message' => translate('Product removed from cart successfully.')
            ]);
        }
    ...
    public function toggleCart(Request $request)
{
    ...
    // Find if product (with variant) exists in cart
    $cartItem = Cart::when($user_id, function ($query) use ($user_id) {
            return $query->where('user_id', $user_id);
        }, function ($query) use ($temp_user_id) {
            return $query->where('temp_user_id', $temp_user_id);
        })
        ->where('product_id', $product_id)
        ->when($variant, fn($q) => $q->where('variation', $variant))
        ->first();
  ...
  public function get_all_product_ids()
    {
        $ids=Cart::where('user_id',auth()->user()->id)->pluck('product_id');
